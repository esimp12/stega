...
from stega_portfolio.bootstrap import service_lifespan
from stega_portfolio.config import PortfolioConfig, create_config, create_logger
from stega_portfolio.domain.error import AppError, ConflictError, ResourceNotFoundError


def create_app(config: PortfolioConfig) -> Quart:
    app = Quart(__name__)
...
class StegaQuartApp:
    ...
    def init_app(self, config: PortfolioConfig, app: Quart) -> None:
        app.extensions = getattr(app, "extensions", {})

        @app.while_serving
        async def _manage_service() -> Awaitable[None]:
            async with service_lifespan(config, start_orm=True) as lifespan:
